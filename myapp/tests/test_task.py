...
from myapp.models import User, Project, Task, Document
...
class TaskAPITestCase(TestCase):
    ...
    def test_upload_document_by_non_assignee_or_assigned_user(self):
        ...
        response = self.client.post(reverse('task-upload-document', kwargs={'pk': self.task.id}), document_data)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_view_documents(self, user=None):
        """Helper method for document viewing tests."""
        if not user:
            user = self.manager_user
    ...
    def test_delete_document(self, user=None):
        ...
        response = self.client.delete(reverse('task-delete-document', kwargs={'pk': self.task.id, 'document_id': document.id}))
        return response
